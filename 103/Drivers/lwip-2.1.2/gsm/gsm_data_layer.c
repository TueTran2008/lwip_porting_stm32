...
void do_unlock_band(gsm_response_event_t event, void *resp_buffer)
{
    ...
    case 0:
    {
        gsm_hw_send_at_cmd("AT+QCFG=\"nwscanseq\",4,1\r\n", "OK\r\n", "", 1000, 10, do_unlock_band);
    }
    break;
...
{
//	uint8_t *buffer = NULL;
//	uint16_t u16len = (uint16_t) len;
//	int16_t remain = 0;
//	if( User_Uart_Received(USER_UART_CHANNEL_USART1, &data, u16len, &remain) == E_OK)
//	{
//		gsm_hw_layer_uart_fill_rx(data, (len + (uint32_t)remain));
//	}
//	strncpy(data,buffer,(len + (uint32_t)remain));
